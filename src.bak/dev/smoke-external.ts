...
import { runCliAdapter } from '../cli/external/adapter.js';
...
async function main() {
  ...
  try {
    ...
    await runCliAdapter({ program: 'node', provider: 'anthropic', args: ['-e', code] });
    ...
    if (/No authentication token found/i.test(msg)) {
      console.error('[smoke-external] Skipping: no token available. Run `copilot-cli auth login` first and retry.');
